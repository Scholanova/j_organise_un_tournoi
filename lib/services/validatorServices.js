...
const validatorService = {
    createCompetition: (joiCompetitionData) => {
        ...
        const competitionSchema = Joi.object({
            ...
            participants: Joi.array().items(Joi.string().min(4, 'utf8').required()).min(parseInt(joiCompetitionData.nb_participant)).max(parseInt(joiCompetitionData.nb_participant)).required()
        })

...
router.post('/new', function (req, res, next) {
    ...
    return validatorServices.createCompetition(competitionData,participants)
        .then (() => {
        ...
        .catch((error) => {
            ...
            } else if(error instanceof TotalParticipantDifferentFromNbParticipant){
                console.log(error.message)
                res.render('competition/new', {
                    bigProblem: error.message
                }) 
            }
            else {
                next(error)
